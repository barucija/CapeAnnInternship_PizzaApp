...
async function getAll(){
  const rows = await db.query(
    `SELECT pizza.id, pizza.name, rating, price, description, picture_link, category.id as category_id, category.name as category_name 
     FROM pizza 
...
async function getByCategory(category_id){
    const rows = await db.query(
      `SELECT pizza.id, pizza.name, rating, price, description, picture_link, category.name as category_name
      FROM pizza 
      inner join category on pizza.category_id = category.id
...
async function create(pizza){
    const result = await db.query(
      `INSERT INTO pizza 
      (name, rating, price, description, picture_link, category_id)
      VALUES 
      ("${pizza.name}", ${pizza.rating}, ${pizza.price}, "${pizza.description}", "${pizza.picture_link}", ${pizza.category_id})`
    );
...
//update function updates one pizza in the databse
  async function update(id, pizza){
    const result = await db.query(
      ...
      description="${pizza.description}", picture_link="${pizza.picture_link}", category_id=${pizza.category_id} 
      WHERE id=${id}` 
    );
  
    let message = 'Error in updating pizza';
  
    if (result.affectedRows) {s
